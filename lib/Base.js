  ...
  Base = (function() {
    ...
    Base.prototype.getPathsAsync = function(fn, type, mask, dir) {
      ...
      return fs.readdir(dir, function(err, read) {
        ...
        if (err) {
          return fn(_this.findFirst === true ? null : paths);
        } else {
          nextPaths = [];
          for (_i = 0, _len = read.length; _i < _len; _i++) {
            _path = read[_i];
          ...
          files = {};
          return async.each(nextPaths, function(item, cb) {
            ...
            if (subDirectories.length === 0) {
              ...
              return async.each(subDirectories, function(item, cb) {
                return _this.getPathsAsync(function(result) {
                  if (_this.findFirst === true && typeof result === 'string') {
                    fn(result);
                    return cb(new Error('Fake error'));
                  } else if (_this.findFirst === true && result === null) {
                    return cb();
                  } else {
                    paths = paths.concat(result);
                    return cb();
    ...
    Base.prototype.getPathsFromParentsAsync = function(fn, mask, type) {
      ...
      directory = this.directory;
      paths = [];
      return this.getPathsAsync(function(_paths) {
        var depth, directories, i, match, _i, _ref;
        console.log(_paths);
        paths = _paths;
        if (_this.findFirst === true && typeof paths === 'string') {
          return fn(paths);
        } else {
          _this.exclude(directory);
          if (_this.up === true) {
            depth = directory.match(/\//g).length;
          } else if (typeof _this.up === 'string') {
            if (_this.up === directory) {
              fn(_this.findFirst === true ? null : paths);
              return null;
            }
            match = path.relative(_this.up, directory).match(/\//g);
            depth = match === null ? 2 : match.length + 2;
          } else {
            depth = _this.up - 1;
          }
          ...
          for (i = _i = 0, _ref = depth - 1; 0 <= _ref ? _i <= _ref : _i >= _ref; i = 0 <= _ref ? ++_i : --_i) {
            ...
            _this.exclude(directories[directories.length - 1]);
          }
          return async.each(directories, function(item, cb) {
            return _this.getPathsAsync(function(result) {
              if (_this.findFirst === true && typeof result === 'string') {
                fn(result);
                return cb(new Error('Fake error'));
